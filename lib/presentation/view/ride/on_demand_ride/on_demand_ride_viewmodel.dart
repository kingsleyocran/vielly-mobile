...
class OnDemandRideViewModel extends ViewModel {
  ...
  double _homeSheetHeight = 395; //250;
  ...
  double _busTripOngoingHeight = 0.0; //150
  get busTripOngoingHeight => _busTripOngoingHeight;
  ...
  get availableSeats => _availableSeats;

  SearchTripBusDetails? _searchTripBusDetails;
  get searchTripBusDetails => _searchTripBusDetails;
  ...
  double? _chargeBus;
  get chargeBus => _chargeBus;

  bool? _rideTypeSelector = true;
  get rideTypeSelector => _rideTypeSelector;
  ...
  Future initialiseOnModelReady(context) async{
    ...
    await triggerSearchTrip(context);
  ...
  Future initializeSearchTrip(pickup, destination) async {
    ...
    searchTripBusDetails = (await getTripRepositoryImpl.postSearchTripBus(data));

    if(searchTripDetails != null && searchTripBusDetails != null){
      _searchTripDetails = searchTripDetails;
      _searchTripBusDetails = searchTripBusDetails;

      _charge = double.parse(searchTripDetails.charge);
      _chargeBus = double.parse(searchTripBusDetails.charge);
  ...
  Future polyLineBusPickupToBusStop(BuildContext context) async{
  ...
  Future polyLinePickupTerminalToBusStop(BuildContext context) async{
    ...
    print('SEARCH DATA###############################');
    var searchTripData = rideTypeSelector ? searchTripDetails : searchTripBusDetails;
  ...
  Future polyLineUserToBusStop(BuildContext context) async{
    ...
    var destination = rideTypeSelector ? searchTripDetails : searchTripBusDetails;
  ...
  triggerSelectRideType(context) async{
    //rideTypeSelector = true: Curver
    //rideTypeSelector = false: Shuttle Share

    if(rideTypeSelector){
      triggerConfirmSeats(context);
    }else{
      await bookBusTrip();
    }
  ...
  triggerConfirmTrip() async{
    ...
    if (rideTypeSelector){
      ...
      await confirmBusTrip();
    }

  }

  triggerStartBusTrip() async{

    _markers.clear();
    _polyLines.clear();
    notifyListeners();

    await showBusTripOngoingSheet();

    await driverLocationListener();

    await polyLineBusPickupToBusStop(context);
  ...
  Future showRideDetailedSheet() async{
  ...
  Future showBusTripOngoingSheet() async{
    ...
    _requestRideHeight = 0;
    _driverPanelHeight = 0;
    _tripOngoingHeight = 0;
    _busTripOngoingHeight = 210;//150
    _meetAtPickupOpacity = 0.0;
    _tripTopStatusOpacity = 0.0;
    _driverArrivingHeight = 0;
    _setLocationOpacity = 1.0;
  ...
  void switchRideSelector(){
    if(rideTypeSelector){
      _rideTypeSelector = false;
      notifyListeners();
    }else{
      _rideTypeSelector = true;
      notifyListeners();
    }
  }
  ...
  Future getBookBusData() async{
    ...
    _availableSeats = bookedBusData.availableSeats;
    notifyListeners();
  }
  ...
  Future bookBusTrip() async{
    ...
    data['lane'] = "0PEKOsoqq6bvgO4wte3c";
    data['date'] = "05/08/2021";
    data['time'] = '18:00 PM';
    data['pickup'] = searchTripBusDetails.pickupID;
    data['dropoff'] = searchTripBusDetails.dropoffID;
    ...
    if(isSuccess){
      ...
      await triggerConfirmSeats(context);
  ...
  Future confirmBusTrip() async{
    ...
    if(value != false){
      ...
      if(isPrimaryMethod != null){

        if(isPrimaryMethod.provider == 'vodafone'){
